...
const isCiLike = Boolean(process.env.CI || process.env.CODEX_CI);
...
type NoiseApplyResponseBody = {
  muted?: Array<{ id?: string }>;
  skipped?: unknown[];
  failed?: unknown[];
};

type NoisePreviewResponseBody = {
  safeCandidates?: Array<{ id?: string }>;
  protected?: unknown[];
  missingSummary?: unknown[];
  notNoise?: unknown[];
};

function idsFromItems(items: Array<{ id?: string }> | undefined) {
  return Array.isArray(items)
    ? items.map((item) => item.id).filter((id): id is string => typeof id === "string" && id.length > 0)
    : [];
}
...
async function openAllFixtureView(page: Page) {
  await page.goto("/?label=all&max=50");
...
test.describe("QA-Strict Unified E2E Tests", () => {
  test.beforeEach(async ({ page }, testInfo) => {
    ...
    try {
      await page.request.post("/api/mailhub/test/reset", { maxRetries: 2, timeout: 30000 });
    } catch (e) {
  ...
  test("2) クリックで選択→detail表示", async ({ page }) => {
    ...
    // 最終的にsubjectが表示される。skeletonは速すぎる環境では観測できない。
  ...
  test("3) ↑↓で選択移動", async ({ page }) => {
    ...
    await expect(page.getByTestId("message-row-selected")).toBeVisible({ timeout: 5000 });
  ...
  test("4) Eで完了→一覧から消える→Uで戻る", async ({ page }) => {
    ...
    // 一覧から消える（行数が減るまで待つ）
    await expect.poll(() => rows.count(), { timeout: 10000, intervals: [100, 250, 500, 1000] }).toBeLessThan(initialCount);
  ...
  test("5) ?でヘルプ→Escで閉じる", async ({ page }) => {
    ...
    await expect(page.getByTestId("shortcut-help")).toBeVisible({ timeout: 5000 });
  ...
  test("6.1) Conversation表示→Expand→本文→Select this conversationで一括選択", async ({ page }) => {
    ...
    const threadRespP = page.waitForResponse((r) =>
      r.url().includes("/api/mailhub/thread?messageId=msg-021") && r.request().method() === "GET" && r.status() === 200,
    );
    await msg021Row.click();
    await threadRespP;
    await expect(page.getByTestId("detail-subject")).toBeVisible({ timeout: 5000 });
  ...
  test("10) 候補バッジ表示→一括ミュート→Undo→戻る", async ({ page }) => {
    ...
    const muteApplyRespP = page.waitForResponse((r) =>
      new URL(r.url()).pathname === "/api/mailhub/noise/apply" && r.request().method() === "POST" && r.status() === 200,
    );
    await bulkMuteButton.click();
    const muteApplyResp = await muteApplyRespP;
    const muteApplyJson = (await muteApplyResp.json().catch(() => ({}))) as NoiseApplyResponseBody;
    const mutedIds = idsFromItems(muteApplyJson.muted);
    expect(mutedIds.length).toBeGreaterThan(0);

    // 一覧件数が減る
    await expect(rows).toHaveCount(initialCount - mutedIds.length, { timeout: 5000 });

    // Undo（ツールバー）
    const undoButton = page.getByTestId("action-undo");
    await expect(undoButton).toBeEnabled({ timeout: 3000 });

    const unmuteResponses = mutedIds.map(() =>
  ...
  test("12) 一括操作（選択→Mute→Undo）", async ({ page }) => {
    ...
    // API成功を待つ（一括muteはnoise/apply 1リクエスト）
    const muteApplyRespP = page.waitForResponse((r) =>
      new URL(r.url()).pathname === "/api/mailhub/noise/apply" && r.request().method() === "POST" && r.status() === 200,
    );
    ...
    // API成功を待つ（実際にmuteされた件数分のunmute）
    const unmuteResponses = mutedIds.map(() => page.waitForResponse((r) =>
      r.url().includes("/api/mailhub/mute") && r.request().method() === "POST" && r.status() === 200
    ));
  ...
  test("13) 一括操作の部分失敗→救済（意図的失敗→リトライ）", async ({ page }) => {
    ...
    // API成功/失敗を待つ（一括muteはnoise/apply 1リクエスト）
    const applyRespP = page.waitForResponse((r) =>
      new URL(r.url()).pathname === "/api/mailhub/noise/apply" && r.request().method() === "POST",
      { timeout: 10000 },
    );
  ...
  test("11) 担当者操作（未割当→担当→解除）", async ({ page }) => {
    ...
    // Step 114: Assignボタンが表示される（即座割当に変更）
    ...
    // Assignボタンをクリック（即座に自分に割当、ポップアップなし）
    ...
    // Unassignボタンをクリック（即座に解除、ポップアップなし）
  ...
  test("14) Activityパネル（操作ログ表示＋フィルタ＋経過時間）", async ({ page }) => {
    ...
    const activityReloadP = page
      .waitForResponse((r) => r.url().includes("/api/mailhub/activity") && r.request().method() === "GET", {
        timeout: 5000,
      })
      .catch(() => null);
  ...
  test("28) テンプレ挿入→下書きに反映→コピーできる", async ({ page }) => {
    ...
    await expect(replyPanel.getByTestId("reply-route")).toHaveText("rakuten_rms", { timeout: 5000 });
    const replyBody = replyPanel.getByTestId("reply-body");
    await expect(replyBody).toBeVisible({ timeout: 10000 });
    ...
    const templatesRespP = page
      ...
      .catch(() => null);
    await replyPanel.getByTestId("reply-template-select").click();

    const picker = page.getByTestId("template-picker");
    await expect(picker).toBeVisible({ timeout: 5000 });
    await templatesRespP;

    const acknowledgedTemplate = picker.getByTestId("reply-template-item-acknowledged");
    await expect(acknowledgedTemplate).toBeVisible({ timeout: 10000 });
    await acknowledgedTemplate.click();
    await expect(picker.getByTestId("reply-template-preview-acknowledged")).toContainText(
      "お問い合わせありがとうございます",
      { timeout: 5000 },
    );
    
    // 挿入ボタンをクリック（Step46: 選択だけでは挿入されない）
    const insertButton = picker.getByTestId("reply-template-insert-acknowledged");
    await expect(insertButton).toBeVisible({ timeout: 5000 });
    await insertButton.click();

    await expect(picker).not.toBeVisible({ timeout: 5000 });
    await expect(replyBody).toHaveValue(/お問い合わせありがとうございます/, { timeout: 10000 });
    await expect(replyPanel.getByTestId("reply-template-applied")).toContainText("acknowledged", { timeout: 5000 });

    // コピー（失敗してもUIは壊れないことが目的）
    const copyButton = replyPanel.getByTestId("reply-copy-template");
  ...
  test("29) Views: サイドバーに表示され、切替で一覧が再取得される（mine）", async ({ page }) => {
    ...
    const mineViewItem = page.getByTestId("view-item-mine");
    if (!(await mineViewItem.isVisible().catch(() => false))) {
      await navViews.locator("details").click();
      await expect(mineViewItem).toBeVisible({ timeout: 3000 });
    }
    await mineViewItem.click();
    await listRespP;
  ...
  test("30) Views Palette: Cmd/Ctrl+Shift+Kで開いてViewsを切替できる", async ({ page }) => {
    ...
    // "mine" view を検索して選択する（日本語表示名にも一致する安定した既定View）
    await palette.locator("input").fill("自分");
    const listRespP = page.waitForResponse((r) =>
      r.url().includes("/api/mailhub/list") && r.request().method() === "GET" && r.status() === 200
    );
    const minePaletteItem = palette.locator('[data-testid="views-palette-item-mine"]');
    await expect(minePaletteItem).toBeVisible({ timeout: 10000 });
    await minePaletteItem.click();
    await listRespP;

    // パレットが閉じられることを確認（view選択が完了した証）
    await expect(palette).toBeHidden({ timeout: 5000 });
    
    // URLが更新されることを確認（view=mineが含まれる）
    // window.history.replaceStateによるURL変更を検知するため、waitForFunctionを使用
    // パレットが閉じられた後、URL更新を待つ
    await page.waitForFunction(
      () => window.location.href.includes("view=mine"),
      { timeout: 5000 }
    ).catch(() => {
...
test("20) ラベル手動付与→行にpill表示→解除", async ({ page }) => {
  ...
  const list = page.getByTestId("message-list");
  const targetRow = list.getByTestId("message-row").first();
  await expect(targetRow).toBeVisible({ timeout: 5000 });
  const targetId = await targetRow.getAttribute("data-message-id");
  if (!targetId) throw new Error("target row has no data-message-id");
  const targetRowById = list.locator(`[data-message-id="${targetId}"]`);

  // チェックしてツールバー操作を有効化
  await page.getByTestId(`checkbox-${targetId}`).check();
  ...
  const newVipInput = pop.getByTestId("label-new-input");
  await newVipInput.fill("");
  await newVipInput.pressSequentially("VIP", { delay: 10 });
  await expect(newVipInput).toHaveValue("VIP");
  await pop.getByTestId("label-new-add").click();

  // VIPを付与。成功toastは /labels/apply 完了後にだけ出るため、response取りこぼしに依存しない。
  const vipButton = pop.getByRole("button", { name: /VIP/ });
  await expect(vipButton).toBeVisible({ timeout: 15000 });
  await vipButton.click();
  await expect(page.getByTestId("toast")).toContainText("ラベルを付与しました", { timeout: 15000 });
  const vipPill = targetRowById.getByTestId("user-label-pill").filter({ hasText: "VIP" });
  await expect(vipPill.first()).toBeVisible({ timeout: 5000 });
  const refreshAfterApplyP = page.waitForResponse(
    (r) => r.url().includes("/api/mailhub/list") && r.request().method() === "GET" && r.status() === 200,
    { timeout: 10000 },
  ).catch(() => null);
  await page.getByTestId("header-refresh").click();
  await refreshAfterApplyP;
  await expect(vipPill.first()).toBeVisible({ timeout: 5000 });
  ...
  await expect(page.getByTestId("toast")).toContainText("ラベルを解除しました", { timeout: 15000 });
  await expect(targetRowById.getByTestId("user-label-pill").filter({ hasText: "VIP" })).toHaveCount(0, { timeout: 5000 });
  const refreshAfterRemoveP = page.waitForResponse(
    (r) => r.url().includes("/api/mailhub/list") && r.request().method() === "GET" && r.status() === 200,
    { timeout: 10000 },
  ).catch(() => null);
  await page.getByTestId("header-refresh").click();
  await refreshAfterRemoveP;
  await expect(targetRowById.getByTestId("user-label-pill").filter({ hasText: "VIP" })).toHaveCount(0, { timeout: 5000 });
});
...
test("21) 単体でラベル+自動ルール作成→一覧再取得→同一fromの別メールに自動でpillが付く", async ({ page }) => {
  ...
  const pop = page.getByTestId("label-popover");
  await expect(pop).toBeVisible({ timeout: 3000 });
  const newAutoInput = pop.getByTestId("label-new-input");
  await newAutoInput.fill("");
  await newAutoInput.pressSequentially("AUTO", { delay: 10 });
  await expect(newAutoInput).toHaveValue("AUTO");
  await pop.getByTestId("label-new-add").click();
  await expect(pop.getByRole("button", { name: /AUTO/ })).toBeVisible({ timeout: 15000 });
  // ルールチェック（単体選択のみ表示）
  ...
  await expect(page.getByTestId("toast")).toContainText("ラベルを付与しました（ルール保存）", { timeout: 15000 });
...
test("31) Support Drawer（Accessガイド + Support Bundle Copy）", async ({ page }) => {
  ...
  // Support Bundleをコピー（診断情報の読み込み完了は not.toBeDisabled で決定論的に待つ）
...
test("34) Ops Board（朝会ビュー/滞留ゼロ）", async ({ page }) => {
  ...
  // セクション or 空状態のいずれかが描画されるまで決定論的に待つ（応答は受信済みだが React 描画は非同期）
  await page.locator(
    'text=/今返す・特に長い|今返す・長く残っている|返事待ち・特に長い|返事待ち・長く残っている|誰かが取る・特に長い|誰かが取る・長く残っている|滞留メールはありません/'
  ).first().waitFor({ state: "visible", timeout: 10000 });
  
  // 少なくとも1つのセクションが表示される（fixturesに古いメールがあるため）
  const hasAnySection = await Promise.race([
    page.getByText(/今返す・特に長い/).isVisible().then(() => true),
    page.getByText(/今返す・長く残っている/).isVisible().then(() => true),
    page.getByText(/返事待ち・特に長い/).isVisible().then(() => true),
    page.getByText(/返事待ち・長く残っている/).isVisible().then(() => true),
    page.getByText(/誰かが取る・特に長い/).isVisible().then(() => true),
    page.getByText(/誰かが取る・長く残っている/).isVisible().then(() => true),
  ...
  if (hasAnySection) {
    ...
    if (await firstItem.isVisible().catch(() => false)) {
      ...
      // Drawerが閉じる
      await expect(opsDrawer).not.toBeVisible({ timeout: 10000 });
      
      // 行クリックでDrawerが閉じることがこのテストの主目的。
      // 遷移先ビューが空になるケースはfixturesの内容に依存するため、詳細表示は必須にしない。
    }
  }
...
test("35) Handoff（開く→preview→Copy→toast→閉じる）", async ({ page }) => {
  ...
  await drawer.getByTestId("handoff-close").click();
...
test("41) Rule Ops（enabled toggle→Apply→stats更新）", async ({ page }) => {
  ...
  if (labelCount === 0) {
    ...
    await expect(page.getByTestId("label-delete-MailHub/Label/test-label")).toBeVisible({ timeout: 10000 });
  }
  
  // Rulesタブに移動
  await page.getByTestId("settings-tab-rules").click();
  await expect(page.getByTestId("settings-panel-rules")).toBeVisible({ timeout: 5000 });
...
test("Step50-1) 連続クリックでdetailが逆転しない（abortの効果）", async ({ page }) => {
  ...
  const rows = page.getByTestId("message-list").getByTestId("message-row");
...
test("Step50-2) Done後に次メールのdetailがすぐ見える（prefetchの効果）", async ({ page }) => {
  ...
  const detailSubject = page.locator('[data-testid="detail-subject"]');
  await expect
    .poll(async () => (await detailSubject.textContent().catch(() => null)) ?? "", {
      timeout: 5000,
...
test("Step51) Search v2（Gmail検索式でサーバ検索＋URL共有＋操作継続）", async ({ page }) => {
  ...
  const searchHitRow = page.getByTestId("message-list").locator('[data-message-id="msg-031"]');
  await expect(searchHitRow).toBeVisible({ timeout: 5000 });
  
  // そのメールを開いて detail が出る
  await searchHitRow.click();
  await expect(page.locator('[data-testid="detail-subject"]')).toBeVisible({ timeout: 3000 });
  ...
  await expect(searchHitRow).not.toBeVisible({ timeout: 3000 });
  ...
  // Undo後の検索結果再取得は環境タイミングに依存するため、検索状態維持を主証拠にする。
  const msg031Count = await searchHitRow.count();
  if (msg031Count === 0) {
...
test("Step52) Work Queues（Queue作成→適用→結果→削除）", async ({ page }) => {
  ...
  await expect(page.getByTestId("settings-panel-queues")).toBeVisible({ timeout: 10000 });
  await expect(page.getByTestId("queue-create")).toBeVisible({ timeout: 15000 });
  ...
  const queueCreateRespP = page.waitForResponse((r) =>
    r.url().endsWith("/api/mailhub/queues") && r.request().method() === "POST" && r.status() === 200,
    { timeout: 10000 }
  );
  await page.getByTestId("queue-create").click();
  await queueCreateRespP; // 作成APIの完了を待つ
  ...
  const queueItem = page.locator('[data-testid^="queues-item-"]').filter({ hasText: "SearchHit" }).first();
  await expect(queueItem).toBeVisible({ timeout: 3000 });
  await queueItem.click();

  // 5. URLに q= が入っていること
  await expect(page).toHaveURL(/q=/, { timeout: 3000 }); // URL更新（q=反映）を待つ
  const url = page.url();
  expect(url).toContain("q=");
  expect(url).toContain("SEARCH_HIT");

  // 6. message-list が絞られていること（msg-031が表示される）
  await expect(page.getByTestId("message-list").locator('[data-message-id="msg-031"]')).toBeVisible({ timeout: 5000 });
  ...
  await page.getByTestId("settings-tab-queues").click();
  // SearchHitキューを探して削除
  const queueRow = page.locator('[data-testid^="queue-row-"]').filter({ hasText: "SearchHit" }).first();
  ...
  await expect(queueRow).not.toBeVisible({ timeout: 10000 });
});
...
test("Step54-2) Handoff Note（Assignee変更時に引き継ぎメモ）", async ({ page }) => {
  ...
  // Step 114: 2. まず自分に担当を割り当て（即座割当に変更）
  const assignButton = page.getByTestId("action-assign");
  ...
  // 3. 詳細ペイン内の担当者pillをクリックしてAssigneeSelectorを開く（引き継ぎメモを入力）
  const detailPane = page.getByTestId("detail-pane");
  const assigneePill = detailPane.getByTestId("assignee-pill").first();
  await expect(assigneePill).toBeVisible({ timeout: 5000 });
  await assigneePill.click({ force: true }); // force: true でポインターインターセプトを回避
...
test("Step56-1) Reply Complete Macro（楽天メールで返信完了→Done実行→Activity記録）", async ({ page }) => {
  ...
  await expect(confirmModal).toContainText("対応済み", { timeout: 3000 });
...
test("Step58-2) Macro: Take+Done（選択→Macroボタン→Take+Done）", async ({ page }) => {
  ...
  // 1) 表示中の未処理メールを選択
  const list = page.getByTestId("message-list");
  const targetRow = list.locator('[data-testid="message-row"]').first();
  ...
  // 3) 対象メールが一覧から消えた（またはDone状態になった）ことを確認
  // Archiveでメッセージはtodoからdoneに移動するので、現在のINBOXビューから消える
  await expect.poll(
    async () => {
      const target = list.locator(`[data-message-id="${targetId}"]`);
...
test("Step101-1) tag付与→一覧pill→tag検索で絞れる", async ({ page }) => {
  ...
  const tagInput = page.getByTestId("work-tag-input");
  const tagAdd = page.getByTestId("work-tag-add");
  const tagChip = page.getByTestId(`work-tag-chip-${tagSlug}`);
  await expect(tagInput).toBeEditable({ timeout: 10000 });
  await expect(tagAdd).toBeEnabled({ timeout: 10000 });
  await expect.poll(async () => {
    if (await tagChip.isVisible().catch(() => false)) return true;
    await tagInput.fill(tagSlug);
    await expect(tagInput).toHaveValue(tagSlug, { timeout: 3000 });
    await tagAdd.click();
    return tagChip.isVisible().catch(() => false);
...
test("Step64-1) Team View（admin only）→Team候補クリック→担当一覧表示", async ({ page }) => {
  ...
  // 1) 担当者セクションが表示される
  const teamSection = page.getByTestId("label-assignee");
  await expect.poll(
    async () => teamSection.isVisible().catch(() => false),
    { timeout: 10000, intervals: [500, 1000, 2000] }
  ).toBe(true);
  
  // 2) 担当者候補の最初のアイテムをクリック
  const firstTeamItem = page.locator('[data-testid^="assignee-item-"]').first();
  ...
  const list = page.getByTestId("message-list");
  const emptyMessage = page.getByText("この条件では0件です");
  // どちらかが表示されればOK
  await expect.poll(
...
test("Step73-1) ラベル付与/解除の楽観更新（pill即時反映 + 二重クリック防止）", async ({ page }) => {
  ...
  const targetRow = list.getByTestId("message-row").first();
  await expect(targetRow).toBeVisible({ timeout: 5000 });
  const targetMessageId = await targetRow.getAttribute("data-message-id");
  if (!targetMessageId) throw new Error("message row must expose data-message-id");
  await page.getByTestId(`checkbox-${targetMessageId}`).check();
  ...
  if (!vipBtnVisible) {
    const newVipInput = labelPop.getByTestId("label-new-input");
    await newVipInput.fill("");
    await newVipInput.pressSequentially("VIP", { delay: 10 });
    await expect(newVipInput).toHaveValue("VIP");
    await labelPop.getByTestId("label-new-add").click();
    await expect(vipBtn).toBeVisible({ timeout: 15000 });
  }
  ...
  // 7) pillが表示される（楽観更新）
  const stableTargetRow = list.locator(`[data-message-id="${targetMessageId}"]`);
  const vipPill = stableTargetRow.getByTestId("user-label-pill").filter({ hasText: "VIP" });
  await expect(vipPill.first()).toBeVisible({ timeout: 5000 });
  const refreshAfterApplyP = page.waitForResponse(
    (r) => r.url().includes("/api/mailhub/list") && r.request().method() === "GET" && r.status() === 200,
    { timeout: 10000 },
  ...
  // 9) pillが消える
  await expect(stableTargetRow.getByTestId("user-label-pill").filter({ hasText: "VIP" })).toHaveCount(0, { timeout: 5000 });
  const refreshAfterRemoveP = page.waitForResponse(
    (r) => r.url().includes("/api/mailhub/list") && r.request().method() === "GET" && r.status() === 200,
    { timeout: 10000 },
  ).catch(() => null);
...
test("Step76-1) Assign Pickerで名簿から選択→Assign→pill更新", async ({ page }) => {
  ...
  // Step 114: 3) まず自分に割り当て（即座割当）
  const assignBtn = page.locator('[data-testid="action-assign"]');
  await expect(assignBtn).toBeVisible({ timeout: 3000 });
  const firstAssignRespP = page.waitForResponse(
    (r) => r.url().includes("/api/mailhub/assign") && r.request().method() === "POST",
    { timeout: 15000 },
  );
  await assignBtn.click();
  await firstAssignRespP;
  
  // 4) 詳細ペイン内のassignee pillをクリックしてAssign Selectorを開く
  const detailPane = page.getByTestId("detail-pane");
  const assigneePill = detailPane.getByTestId("assignee-pill").first();
  await expect(assigneePill).toBeVisible({ timeout: 5000 });
  await assigneePill.click({ force: true }); // force: true でポインターインターセプトを回避
...
test("Step77-1) 左ナビに名簿全員が表示され、クリックでフィルタされる", async ({ page }) => {
  ...
  const bobItem = page.getByTestId("assignee-item-bob@vtj.co.jp");
  ...
  await expect(
    page.getByTestId("message-list").or(page.locator("text=この条件では0件です"))
...
test("Step84-1) Settings Auto Rules→Assignee選択→Preview→Apply", async ({ page }) => {
  ...
  const createRespP = page.waitForResponse(
    (r) => new URL(r.url()).pathname === "/api/mailhub/rules" && r.request().method() === "POST" && r.status() === 200,
    { timeout: 10000 }
  );
  await page.getByTestId("rule-create").click();
...
test("Step85-1) Settings→Templates作成→Inbox→テンプレコピー→toast成功", async ({ page }) => {
  ...
  await page.getByTestId("settings-drawer-close").click();
  await expect(page.getByTestId("settings-drawer")).toBeHidden({ timeout: 5000 });
...
test("Step87-1) Snooze→一覧から消える→Snoozedに出る→Unsnoozeで復帰", async ({ page }) => {
  ...
  await page.getByTestId("snooze-preset-明日").click();
  await snoozeRespP;
...
test("Step90-1) Bulk Done 10件以上→confirm表示→OK→API成功", async ({ page }) => {
  ...
  await expect(confirmModal).toContainText("10件のメールを対応済みにします");
...
test("Step92-1) Onboarding 初回でモーダル表示→閉じる→再読み込みで出ない→Helpで出る", async ({ browser }) => {
  ...
  await expect(onboardingModal).toContainText("処理不要と復帰");
...
test("Step93-1) Hover Prefetch: hover→クリックでスケルトンが最小化される（イベント駆動確認）", async ({ page }) => {
  ...
  // 2) 初期選択の隣接prewarm対象外の行を取得する
  expect(rowCount).toBeGreaterThan(6);
  const targetRow = messageRows.nth(6);
  ...
  // 4) hover開始（クリック直後の詳細取得と競合しない遅延でprefetchが開始される）
...
test("Step93-2) Hover Prefetch: 連続hoverで前のリクエストがキャンセルされる", async ({ page }) => {
  ...
  // 3) 初期選択の隣接prewarm対象外の行を使う
  expect(rowCount).toBeGreaterThan(7);
  const row1 = messageRows.nth(6);
  await row1.hover();
  await page.mouse.move(0, 0); // マウスを離脱（タイマーキャンセル）
  
  // 4) 2行目をhover
  const row2 = messageRows.nth(7);
...
test("Step93-3) Mobile layout: 一覧と詳細が全幅で切り替わり横スクロールしない", async ({ page }) => {
  ...
  const detailAssigneePill = page.locator('.mailhub-detail-column button[data-testid="assignee-pill"]').first();
  await expect(detailAssigneePill).toBeVisible({ timeout: 5000 });
  await expect
    .poll(
      async () =>
        page.evaluate(() => {
          ...
          const assignee = document.querySelector('.mailhub-detail-column button[data-testid="assignee-pill"]');
...
test("Step93-3b) Narrow desktop action rail: ツールバーと作業タブが縦割れしない", async ({ page }) => {
  await page.request.post("/api/mailhub/test/reset").catch(() => {});
  await page.addInitScript(() => {
    localStorage.setItem("mailhub-onboarding-shown", "true");
  });
...
test("Step93-3c6) Mail preview interaction: 初回直後と連続切替でフレーム停止しない", async ({ page }) => {
  ...
  const measureSwitch = async (targetId: string, subjectText: string, requiredBodyText: string) => {
    ...
    const interactionReportP = page.evaluate(
      ...
      { targetId, subjectText, pollMs: isCiLike ? 3000 : 2000 },
    );
    ...
    expect(interactionReport.bodyReadyAt ?? 9999).toBeLessThan(isCiLike ? 3000 : 2000);
    expect(interactionReport.maxFrameGap).toBeLessThan(isCiLike ? 1000 : 750);
    // No catastrophic single block (real freeze), and at most one moderate dev-mode commit per switch.
    expect(interactionReport.longTasks.filter((duration) => duration >= (isCiLike ? 1200 : 800))).toEqual([]);
...
test("Step93-4) Channel scope bar: 専用宛先と関連候補検索を分けて表示する", async ({ page }) => {
  ...
  const scopeBar = page.getByTestId("channel-scope-bar");
  await expect(scopeBar).toBeVisible({ timeout: 5000 });
  await expect(scopeBar).toContainText("StoreA");
  await expect(scopeBar).toContainText("専用宛先: shop-a@vtj.co.jp");
  ...
  await expect(page.getByTestId("channel-scope-bar")).not.toBeVisible({ timeout: 5000 });
  await expect(page.getByTestId("message-row").first()).toBeVisible({ timeout: 5000 });
});
...
test("Step93-5) Sender mute preview: 同じ送信元の未処理メールをまとめて処理不要にできる", async ({ page }) => {
  ...
  const previewRespP = page.waitForResponse(
    (r) => new URL(r.url()).pathname === "/api/mailhub/noise/preview" && r.request().method() === "POST" && r.status() === 200,
    { timeout: 30000 },
  );
  await page.getByTestId("action-mute-sender").click();
  const previewResp = await previewRespP;
  const previewJson = (await previewResp.json().catch(() => ({}))) as NoisePreviewResponseBody;
  const safeCandidateIds = idsFromItems(previewJson.safeCandidates);
  expect(safeCandidateIds.length).toBeGreaterThan(1);
  const preview = page.getByTestId("sender-mute-preview");
  await expect(preview).toBeVisible({ timeout: 5000 });
  await expect(preview).toContainText("label-tester@example.com");
  await expect(preview).toContainText(`実行対象 ${safeCandidateIds.length}件`);
  await expect(page.getByTestId("sender-mute-execute")).toBeEnabled({ timeout: 10000 });

  const muteApplyRespP = page.waitForResponse(
    (r) => new URL(r.url()).pathname === "/api/mailhub/noise/apply" && r.request().method() === "POST" && r.status() === 200,
    { timeout: 30000 },
  );
  const reloadAfterMuteP = page.waitForResponse(
    (r) => {
      if (r.request().method() !== "GET" || r.status() !== 200) return false;
      const url = new URL(r.url());
      return url.pathname === "/api/mailhub/list" && url.searchParams.get("q") === "from:label-tester@example.com";
    },
    { timeout: 30000 },
  );
  ...
  const muteApplyResp = await muteApplyRespP;
  const muteApplyJson = (await muteApplyResp.json().catch(() => ({}))) as NoiseApplyResponseBody;
  expect(idsFromItems(muteApplyJson.muted)).toHaveLength(safeCandidateIds.length);
  await reloadAfterMuteP;
  await expect(preview).not.toBeVisible({ timeout: 30000 });
  await expect(page.getByText("この条件では0件です")).toBeVisible({ timeout: 10000 });
});

test("Step93-6) Gmail-like density: 詳細本文を上寄せし、hover行が明確に見える", async ({ page }) => {
  await page.request.post("/api/mailhub/test/reset").catch(() => {});
  await page.addInitScript(() => {
    localStorage.setItem("mailhub-onboarding-shown", "true");
  });
  ...
  const secondRow = page.getByTestId("message-row").nth(1);
  await secondRow.scrollIntoViewIfNeeded();
  await secondRow.hover();
  await expect
    .poll(async () => {
      return await secondRow.evaluate((el) => {
  ...
  const metrics = await page.evaluate(() => {
    const firstRow = document.querySelector('[data-testid="message-row"]');
    ...
    return {
      ...
      sidebarItemHeight:
        document.querySelector('[data-testid="label-status"] [data-testid^="label-item-"]')?.getBoundingClientRect()
          .height ?? 0,
      sidebarSummaryHeight: document.querySelector('[data-testid="nav-views"] summary')?.getBoundingClientRect().height ?? 0,
      sidebarOverflow: (() => {
        const scrollArea = document.querySelector(".mailhub-sidebar .flex-1.overflow-y-auto");
  ...
  expect(metrics.sidebarItemHeight).toBeGreaterThan(0);
  expect(metrics.sidebarItemHeight).toBeLessThanOrEqual(28);
  expect(metrics.sidebarSummaryHeight).toBeLessThanOrEqual(28);
  expect(metrics.sidebarOverflow).toBeLessThanOrEqual(0);
});
...
test("Step93-7) Gmail-like attachments: 一覧と詳細末尾に添付ファイルが自然に出る", async ({ page }) => {
  ...
  await expect(page.getByTestId("attachment-open")).toHaveAttribute("href", /messageId=msg-001/);
  await expect(page.getByTestId("attachment-preview")).toHaveCount(0);
  await expect(page.getByTestId("attachment-download")).toBeVisible();

  const inlineHref = await page.getByTestId("attachment-open").getAttribute("href");
  expect(inlineHref).toBeTruthy();
  const inlineResponse = await page.request.get(inlineHref!);
  expect(inlineResponse.status()).toBe(200);
  expect(inlineResponse.headers()["x-content-type-options"]).toBe("nosniff");
  expect(inlineResponse.headers()["content-security-policy"]).toBe("sandbox");
  expect(inlineResponse.headers()["content-disposition"]).toContain("inline;");

  const downloadHref = await page.getByTestId("attachment-download").getAttribute("href");
  expect(downloadHref).toBeTruthy();
  expect(downloadHref).toContain("disposition=attachment");
  ...
  expect(downloadResponse.headers()["content-security-policy"]).toBe("sandbox");
  ...
  const download = await downloadP;
  expect(download.suggestedFilename()).toBe("注文確認書_123-4567890.pdf");
  const downloadPath = await download.path();
  if (!downloadPath) throw new Error("attachment download.path() is null");
  const fs = await import("fs/promises");
  const downloaded = await fs.readFile(downloadPath, "utf-8");
  expect(downloaded).toContain("MailHub test attachment");
  expect(downloaded).toContain("attachmentId: att-msg-001-invoice");

  const positions = await page.evaluate(() => {
    const attachments = document.querySelector('[data-testid="detail-attachments"]')?.getBoundingClientRect();
    const notes = document.querySelector('#section-notes')?.getBoundingClientRect();
    return {
      attachmentsTop: attachments?.top ?? 0,
      notesTop: notes?.top ?? 0,
      detailText: document.querySelector('[data-testid="detail-pane"]')?.textContent ?? "",
      threadPaneCount: document.querySelectorAll('[data-testid="thread-pane"]').length,
    };
  });
  expect(positions.attachmentsTop).toBeGreaterThan(0);
  expect(positions.notesTop).toBeGreaterThan(positions.attachmentsTop);
  expect(positions.detailText).not.toContain("No messages");
  await expect(page.getByTestId("thread-pane")).toHaveCount(0, { timeout: 5000 });
});
...
test("Step94-1) Done: 失敗時rollback / 成功時保持（イベント駆動）", async ({ page }) => {
  const rollbackTargetId = "msg-021";
  // 1) archiveを失敗させる設定で初期化
  await page.request.post("/api/mailhub/test/reset", {
    data: { fail: { endpoint: "archive", ids: [rollbackTargetId] } },
  });
  await page.addInitScript(() => {
    localStorage.setItem("mailhub-onboarding-shown", "true");
  });
  ...
  // 2) rollback対象を選択してDone
  const targetRow = page.getByTestId("message-list").locator(`[data-message-id="${rollbackTargetId}"]`);
  await expect(targetRow).toBeVisible({ timeout: 5000 });
  await targetRow.click();
  ...
  const successRow = page.getByTestId("message-list").getByTestId("message-row").first();
  await expect(successRow).toBeVisible({ timeout: 5000 });
  const successId = await successRow.getAttribute("data-message-id");
  if (!successId) throw new Error("success row must expose data-message-id");
  ...
  // 5) 成功時は一覧から消える
  await expect(page.getByTestId("message-list").locator(`[data-message-id="${successId}"]`)).toBeHidden({ timeout: 3000 });
});

// ========== Step 104: Paging E2E（強制nextPageTokenでLoad more検証） ==========
test("Step104-1) storesでpartial表示→Support Bundle診断→Load moreで追記される", async ({ page }) => {
  await page.request.post("/api/mailhub/test/reset", { data: { readOnly: false } }).catch(() => {});
  await page.addInitScript(() => {
    localStorage.setItem("mailhub-onboarding-shown", "true");
  });
  ...
  const makeMessage = (id: string, subject: string) => ({
    ...
    threadId: `thread-${id}`,
    subject,
    from: "Store Source <shop-a@vtj.co.jp>",
  ...
  const firstPage = [
    makeMessage("msg-001", "Forced stores page 1 - A"),
    makeMessage("msg-002", "Forced stores page 1 - B"),
    makeMessage("msg-003", "Forced stores page 1 - C"),
  ];
  const secondPage = [
    makeMessage("msg-004", "Forced stores page 2 - D"),
    makeMessage("msg-005", "Forced stores page 2 - E"),
  ];
  const listRequests: Array<{ label: string | null; pageToken: string | null; max: string | null }> = [];
  ...
  await page.route("**/api/mailhub/list**", async (route) => {
    ...
    listRequests.push({ label, pageToken, max });
    const isSecondPage = pageToken === "stores-page-2";
    await route.fulfill({
      status: 200,
      ...
      body: JSON.stringify({
        ...
        nextPageToken: isSecondPage ? undefined : "stores-page-2",
        meta: {
          loadedCount: isSecondPage ? secondPage.length : firstPage.length,
          max: Number(max ?? 5),
          hasMore: !isSecondPage,
          pageTokenApplied: isSecondPage,
          sourceScope: {
            channel: { id: "stores", label: "ストア全部", addresses: [] },
            sourceChannels: [
              { id: "store-a", label: "StoreA", addresses: ["shop-a@vtj.co.jp"] },
              { id: "store-b", label: "StoreB", addresses: ["shop-b@vtj.co.jp"] },
              { id: "store-c", label: "StoreC", addresses: ["shop-c@vtj.co.jp"] },
            ],
            sourceAddresses: ["shop-a@vtj.co.jp", "shop-b@vtj.co.jp", "shop-c@vtj.co.jp"],
            isAggregate: true,
          },
        },
      }),
  ...
  await page.getByTestId("label-item-stores").click();
  await expect(page.locator('[data-message-id="msg-001"]')).toContainText("Forced stores page 1 - A", { timeout: 10000 });
  await expect(page.getByTestId("message-row")).toHaveCount(3);

  const scopeBar = page.getByTestId("channel-scope-bar");
  await expect(scopeBar).toBeVisible({ timeout: 5000 });
  await expect(scopeBar).toContainText("ストア全部");
  await expect(scopeBar).toContainText("3件読み込み済み");
  await expect(scopeBar).toContainText("続きあり");
  await expect(scopeBar).toContainText("全件ではありません");

  await page.getByTestId("action-help").click();
  const helpDrawer = page.getByTestId("help-drawer");
  await expect(helpDrawer).toBeVisible({ timeout: 5000 });
  await helpDrawer.getByTestId("help-tab-support").click();
  const supportBundle = helpDrawer.getByTestId("support-bundle-preview");
  await expect(supportBundle).toContainText('"activeLabelId": "stores"', { timeout: 10000 });
  await expect(supportBundle).toContainText('"pageSize": 5');
  await expect(supportBundle).toContainText('"loadedCount": 3');
  await expect(supportBundle).toContainText('"hasMore": true');
  await expect(supportBundle).toContainText('"isPartial": true');
  await expect(supportBundle).toContainText('"sourceAddresses"');
  await page.keyboard.press("Escape");
  await expect(helpDrawer).not.toBeVisible({ timeout: 5000 });

  const nextPageRespP = page.waitForResponse(
    (r) => r.url().includes("/api/mailhub/list") && r.url().includes("pageToken=stores-page-2") && r.request().method() === "GET",
    { timeout: 10000 },
  );
  await page.getByTestId("action-load-more").click();
  await nextPageRespP;

  await expect(page.locator('[data-message-id="msg-005"]')).toContainText("Forced stores page 2 - E", { timeout: 10000 });
  await expect(page.getByTestId("message-row")).toHaveCount(5);
  await expect(page.getByTestId("action-load-more")).toHaveCount(0);
  expect(listRequests.some((request) => request.pageToken === "stores-page-2" && request.max === "5")).toBe(true);
});
...
test("Step108-1) Views: 作成→適用でmsg-021表示→削除で左ナビから消える", async ({ page }) => {
  ...
  const [listResponse] = await Promise.all([
    page.waitForResponse((r) => {
      if (!r.url().includes("/api/mailhub/list") || r.status() !== 200) return false;
      const url = new URL(r.url());
      return url.searchParams.get("q") === "楽天";
    }),
  ...
  expect(messageIds).not.toContain("msg-001");
...
test("Step110-1) rosterに2名登録→メールに2番目をAssign→一覧pillが「他人担当」になる", async ({ page }) => {
  ...
  // 7) 詳細ペイン内のassignee pillをクリックしてモーダルを開く
  const detailPane = page.getByTestId("detail-pane");
  const assigneePill = detailPane.getByTestId("assignee-pill").first();
  await expect(assigneePill).toBeVisible({ timeout: 5000 });
  await assigneePill.click({ force: true }); // force: true でポインターインターセプトを回避
  
  // Assign Modalが表示されることを確認（assignee-selector）
...
async function w2T3aResetAndOpen(
  ...
  await page.goto(`/?label=all&id=${w2T3aMessageId}&max=50`);
  await detailRespP;
