...
test("Step93-5) Sender mute preview: 同じ送信元の未処理メールをまとめて処理不要にできる", async ({ page }) => {
  ...
  const muteRespPs = Array.from({ length: beforeCount }, () =>
    page.waitForResponse(
      (r) => new URL(r.url()).pathname === "/api/mailhub/mute" && r.request().method() === "POST" && r.status() === 200,
      { timeout: 30000 },
    ),
  );
